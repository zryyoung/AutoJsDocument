...
$(document).ready(function() {
    ...
    if (jQuery(".screenshot-slider").length > 0) {
        var titles = [
            '市场界面，可下载和上传项目',
            '工作区界面，可多Tab编辑和打开多个文件',
            '作品：蚂蚁庄园 by TonyJiang',
            '作品：简单词本 by 四五，可在酷安下载', 
            '作品：坦克大战游戏 by 媱枰鴲疄敀渃', 
            '作品：某弹琴工具 by StageGuard', 
        ];
        $('#screenshot-title').text(titles[0]);
        $(".screenshot-slider").on('beforeChange', function(event, slick, currentSlide, nextSlide){
            $('#screenshot-title').text(titles[nextSlide]);
        });
